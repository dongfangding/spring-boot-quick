...
public class MqttPublishSubscribeEventImpl implements MqttPublishSubscribeEvent {
    ...
    public void onPublishComplete(DefaultMqtt5Client defaultMqtt5Client, Mqtt5PublishResult mqtt5PublishResult) {
        if (mqtt5PublishResult.getError().isPresent()) {
            ...
            log.info(
                    "[{}]发布消息成功!",
                    new String(mqtt5PublishResult.getPublish().getPayloadAsBytes(), StandardCharsets.UTF_8)
            );
        }
    }
    ...
    public void onSubscribeComplete(DefaultMqtt5Client defaultMqtt5Client, MqttSubscribe mqttSubscribe,
        ...
        if (throwable != null) {
            log.error(
                    "主题[{}]订阅失败！", mqttSubscribe.getSubscriptions()
                            .stream()
                            .map(mqttSubscription -> mqttSubscription.getTopicFilter().getTopicFilterString())
                            .toArray(), throwable);
        } else {
            ...
            log.info(
                    "主题[{}]订阅成功!", mqttSubscribe.getSubscriptions()
                            .stream()
                            .map(mqttSubscription -> mqttSubscription.getTopicFilter().getTopicFilterString())
                            .toArray());
        }
